...
exports.log = [];
function log (m) { exports.log.push(m); }
...
function requireAsync (base, id, parent, cb) {
  resolve(base, id, parent, function (er, uri) {
    ...
    function firstCompile (er, data) {
      ...
      if ("string" === typeof data && child.ext !== ".js") {
        // if it's not already JS, it can compile to a JS string.
        return child.compile(data, ".js", cb);
      }
      cb(null, child.exports);
    }
    child.load(function (er, data) {
      log("loaded "+child.uri+ " "+data);
      if (er) return cb(er);
      if ("string" === typeof data) {
        child.compile(data, firstCompile);
      } else {
        cb(null, child.exports);
      }
    });
...
function resolve (root, id, parent, cb) {
  ...
  log("look for resolver for "+scheme);
  if (scheme === "file:") {
...
function bootstrap () {
  ...
  registerExtension(".js", "compile", function (content, cb) {
    ...
    if (cb) {
      ...
      log("returning exports "+this.filename);
      this.loaded = true;
      return this.exports;
    }
  });
